...
// --------------------------------------
@group(2) @binding(0) // delay sensor
...
@group(2) @binding(1) // info rec ptos
...
@group(2) @binding(2) // sensors signals sigyy
...
fn velocity_kernel(@builtin(global_invocation_id) index: vec3<u32>) {
    ...
    if(sensor != -1 && sim_int_par.it >= get_delay_rec(sensor)) {
       let value_sens_sigyy: f32 = get_sens_sigyy(sim_int_par.it, sensor) + get_sigmayy(x, y);
       set_sens_sigyy(sim_int_par.it, sensor, value_sens_sigyy);
    }
