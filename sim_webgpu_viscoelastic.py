...
class SimulatorWebGPU(Simulator):
    ...
    def implementation(self):
        ...
        with open('shader_webgpu_viscoelastic.wgsl') as shader_file:
        ...
        params_i32 = np.array([self._nx, self._ny, self._n_steps, self._n_src, self._n_rec, self._n_pto_rec, ord, 0, n_sls, 1 if viscoelastic_attn else 0],
                               dtype=int32)
        ...
        b_r_xx = self._device.create_buffer_with_data(data=np.zeros((self._nx, self._ny, n_sls), dtype=flt32), usage=read_write_mask)

        b_r_yy = self._device.create_buffer_with_data(data=np.zeros((self._nx, self._ny, n_sls), dtype=flt32), usage=read_write_mask)

        b_r_xy = self._device.create_buffer_with_data(data=np.zeros((self._nx, self._ny, n_sls), dtype=flt32), usage=read_write_mask)

        b_r_xx_old = self._device.create_buffer_with_data(data=np.zeros((self._nx, self._ny, n_sls), dtype=flt32), usage=read_write_mask)

        b_r_yy_old = self._device.create_buffer_with_data(data=np.zeros((self._nx, self._ny, n_sls), dtype=flt32), usage=read_write_mask)

        b_r_xy_old = self._device.create_buffer_with_data(data=np.zeros((self._nx, self._ny, n_sls), dtype=flt32), usage=read_write_mask)
        ...
        b_vx = self._device.create_buffer_with_data(data=np.zeros((self._nx, self._ny), dtype=flt32),
                                                    usage=read_write_mask)
        b_vy = self._device.create_buffer_with_data(data=np.zeros((self._nx, self._ny), dtype=flt32),
                                                    usage=read_write_mask)
        b_v_2 = self._device.create_buffer_with_data(data=flt32(0.0), usage=read_write_mask)

        # ----------------- Estresses ---------------------
        b_sigmaxx = self._device.create_buffer_with_data(data=np.zeros((self._nx, self._ny), dtype=flt32),
                                                          usage=read_write_mask)
        b_sigmayy = self._device.create_buffer_with_data(data=np.zeros((self._nx, self._ny), dtype=flt32),
                                                          usage=read_write_mask)
        b_sigmaxy = self._device.create_buffer_with_data(data=np.zeros((self._nx, self._ny), dtype=flt32),
                                                          usage=read_write_mask)

        # Arrays de memoria do simulador
        b_memory_dvx_dx = self._device.create_buffer_with_data(data=np.zeros((self._nx, self._ny), dtype=flt32),
                                                               usage=read_only_mask)
        b_memory_dvx_dy = self._device.create_buffer_with_data(data=np.zeros((self._nx, self._ny), dtype=flt32),
                                                               usage=read_only_mask)
        b_memory_dvy_dx = self._device.create_buffer_with_data(data=np.zeros((self._nx, self._ny), dtype=flt32),
                                                               usage=read_only_mask)
        b_memory_dvy_dy = self._device.create_buffer_with_data(data=np.zeros((self._nx, self._ny), dtype=flt32),
                                                               usage=read_only_mask)
        b_memory_dsigmaxx_dx = self._device.create_buffer_with_data(data=np.zeros((self._nx, self._ny), dtype=flt32),
                                                                     usage=read_only_mask)
        b_memory_dsigmayy_dy = self._device.create_buffer_with_data(data=np.zeros((self._nx, self._ny), dtype=flt32),
                                                                     usage=read_only_mask)
        b_memory_dsigmaxy_dx = self._device.create_buffer_with_data(data=np.zeros((self._nx, self._ny), dtype=flt32),
                                                                    usage=read_only_mask)
        b_memory_dsigmaxy_dy = self._device.create_buffer_with_data(data=np.zeros((self._nx, self._ny), dtype=flt32),
                                                                    usage=read_only_mask)
        ...
        b_sens_sigxx = self._device.create_buffer_with_data(data=np.zeros((self._n_steps, self._n_rec), dtype=flt32),
                                                        usage=read_write_mask)
        b_sens_sigyy = self._device.create_buffer_with_data(data=np.zeros((self._n_steps, self._n_rec), dtype=flt32),
                                                        usage=read_write_mask)
        ...
        sigmayy = np.asarray(self._device.queue.read_buffer(b_sigmayy, buffer_offset=0).cast("f")).reshape(
            (self._nx, self._ny))
        sens_sigmayy = np.array(self._device.queue.read_buffer(b_sens_sigyy).cast("f")).reshape(
