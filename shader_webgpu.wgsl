...
@group(1) @binding(2) // p_2
var<storage,read_write> p_2: f32;

// -------------------------------------
// --- Pressure array access funtions --
// -------------------------------------
@group(1) @binding(3) // pressure field
var<storage,read_write> pressure: array<f32>;

// function to get a pressure array value
fn get_pressure(x: i32, y: i32) -> f32 {
    let index: i32 = ij(x, y, sim_int_par.x_sz, sim_int_par.y_sz);

    return select(0.0, pressure[index], index != -1);
}

// function to set a pressure array value
fn set_pressure(x: i32, y: i32, val : f32) {
    let index: i32 = ij(x, y, sim_int_par.x_sz, sim_int_par.y_sz);
...
// -------------------------------------
@group(1) @binding(4) // memory_dvx_dx field
...
@group(1) @binding(5) // mdvy_dy field
...
@group(1) @binding(6) // mdpressure_dx field
var<storage,read_write> mdpressure_dx: array<f32>;

// function to get a memory_dpressure_dx array value
fn get_mdpressure_dx(x: i32, y: i32) -> f32 {
    let index: i32 = ij(x, y, sim_int_par.x_sz, sim_int_par.y_sz);

    return select(0.0, mdpressure_dx[index], index != -1);
}

// function to set a memory_dpressure_dx array value
fn set_mdpressure_dx(x: i32, y: i32, val : f32) {
    let index: i32 = ij(x, y, sim_int_par.x_sz, sim_int_par.y_sz);

    if(index != -1) {
        mdpressure_dx[index] = val;
    }
}

// ----------------------------------

@group(1) @binding(7) // mdpressure_dy field
var<storage,read_write> mdpressure_dy: array<f32>;

// function to get a memory_dpressure_dy array value
fn get_mdpressure_dy(x: i32, y: i32) -> f32 {
    let index: i32 = ij(x, y, sim_int_par.x_sz, sim_int_par.y_sz);

    return select(0.0, mdpressure_dy[index], index != -1);
}

// function to set a memory_dpressure_dy array value
fn set_mdpressure_dy(x: i32, y: i32, val : f32) {
    let index: i32 = ij(x, y, sim_int_par.x_sz, sim_int_par.y_sz);

    if(index != -1) {
        mdpressure_dy[index] = val;
    }
}
...
// Kernel to calculate pressure
@compute
@workgroup_size(wsx, wsy)
fn pressure_kernel(@builtin(global_invocation_id) index: vec3<u32>) {
    let x: i32 = i32(index.x);          // x thread index
    let y: i32 = i32(index.y);          // y thread index
    ...
    if(x >= id_x_i && x < id_x_f && y >= id_y_i && y < id_y_f) {
        ...
        for(var c: i32 = 0; c < sim_int_par.fd_coeff; c++) {
            vdvx_dx += get_fdc(c) * (get_vx(x + get_idx_ih(c), y) - get_vx(x + get_idx_fh(c), y)) * one_dx;
            vdvy_dy += get_fdc(c) * (get_vy(x, y + get_idx_if(c)) - get_vy(x, y + get_idx_ff(c))) * one_dy;
        }
...
fn velocity_kernel(@builtin(global_invocation_id) index: vec3<u32>) {
    ...
    var id_y_f: i32 = sim_int_par.y_sz - get_idx_if(last);
    if(x >= id_x_i && x < id_x_f && y >= id_y_i && y < id_y_f) {
        var vdpressure_dx: f32 = 0.0;
        for(var c: i32 = 0; c < sim_int_par.fd_coeff; c++) {
            vdpressure_dx += get_fdc(c) * (get_pressure(x + get_idx_if(c), y) - get_pressure(x + get_idx_ff(c), y)) * one_dx;
        }

        let mdpressure_dx_new: f32 = get_b_x(x - offset) * get_mdpressure_dx(x, y) + get_a_x(x - offset) * vdpressure_dx;
        vdpressure_dx = vdpressure_dx/get_k_x(x - offset) + mdpressure_dx_new;
        set_mdpressure_dx(x, y, mdpressure_dx_new);
    ...
    if(x >= id_x_i && x < id_x_f && y >= id_y_i && y < id_y_f) {
        var vdpressure_dy: f32 = 0.0;
        for(var c: i32 = 0; c < sim_int_par.fd_coeff; c++) {
            vdpressure_dy += get_fdc(c) * (get_pressure(x, y + get_idx_ih(c)) - get_pressure(x, y + get_idx_fh(c))) * one_dy;
        }

        let mdpressure_dy_new: f32 = get_b_y_h(y - offset) * get_mdpressure_dy(x, y) + get_a_y_h(y - offset) * vdpressure_dy;
        vdpressure_dy = vdpressure_dy/get_k_y_h(y - offset) + mdpressure_dy_new;
        set_mdpressure_dy(x, y, mdpressure_dy_new);

        let rho: f32 = 0.25 * (get_rho(x, y) + get_rho(x + 1, y) + get_rho(x + 1, y + 1) + get_rho(x, y + 1));
        if(rho > 0.0) {
            let vy: f32 = get_vy(x, y) + vdpressure_dy * dt / rho;
            set_vy(x, y, vy);
        }
    }
}
